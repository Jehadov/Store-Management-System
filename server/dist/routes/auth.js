...
authRouter.post('/seed', async (req, res) => {
    ...
    const { rows } = await pool.query("INSERT INTO users (username, password_hash, role) VALUES ($1,$2,'admin') RETURNING id, username, role", [username, hash]);
...
authRouter.post('/users', requireAuth, requireRole('admin'), async (req, res) => {
    ...
    const hash = await bcrypt.hash(password, 10);
    const { rows } = await pool.query('INSERT INTO users (username, password_hash, role) VALUES ($1,$2,$3) RETURNING id, username, role', [username.trim(), hash, role]);
    res.status(201).json(rows[0]);
});

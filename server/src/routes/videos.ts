...
videosRouter.post('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (!b.url?.trim()) return res.status(400).json({ error: 'url required' });
  const { rows } = await pool.query(
    `INSERT INTO videos (title_en, title_ar, description_en, description_ar, url, thumbnail, is_active, sort_order)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [b.title_en.trim(), b.title_ar?.trim() || '', b.description_en || '', b.description_ar || '',
...
videosRouter.put('/:id', requireAuth, requireRole('admin'), async (req, res) => {
  const b = req.body || {};
  const { rows } = await pool.query(
    `UPDATE videos SET title_en=COALESCE($1,title_en), title_ar=COALESCE($2,title_ar),
    ...
    [b.title_en ?? null, b.title_ar ?? null, b.description_en ?? null, b.description_ar ?? null,

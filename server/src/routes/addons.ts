...
addonsRouter.post('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (!name_en?.trim()) return res.status(400).json({ error: 'name_en required' });
  const { rows } = await pool.query(
    'INSERT INTO addons (name_en, name_ar, extra_price) VALUES ($1,$2,$3) RETURNING *',
    [name_en.trim(), name_ar?.trim() || '', Number(extraPrice) || 0]
  );
...
addonsRouter.put('/:id', requireAuth, requireRole('admin'), async (req, res) => {
  const { name_en, name_ar, extraPrice } = req.body;
  const { rows } = await pool.query(
    'UPDATE addons SET name_en=$1, name_ar=$2, extra_price=$3, updated_at=now() WHERE id=$4 RETURNING *',
    [name_en || '', name_ar || '', Number(extraPrice) || 0, req.params.id]
  );
...
addonsRouter.put('/link/product', requireAuth, requireRole('admin'), async (req, res) => {
  const { productId, addonIds } = req.body as { productId: string; addonIds: string[] };
